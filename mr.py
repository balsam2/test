"""
We are going to be running mapreduce jobs on the wikipedia dataset.  The dataset is available (pre-chunked) on [s3](s3://thedataincubator/coursedata/mrdata/simple/).
...
@QuestionList.add
class Top100WordsEnglishWikipediaNoMetaData(StringNumberListValidateMixin, Question):
  """
  Do the same thing but now for the english [english wikipedia dataset](s3://thedataincubator/coursedata/mrdata/english/).
  """
  def solution(self):
    """
    A list of 100 tuples of word and count using the same format.
    """
    return [("the", 197233964)] * 100
...
@QuestionList.add
class Top100DoubleLinkEnglishWikipedia(TupleNumberListValidateMixin, Question):
  """
  The same thing but for all of English Wikipedia.  This is the real test of how well your algorithm scales!  The results here should look a lot more reasonable.  You should find that there are a diverse set of pairs which are all closely related conceptually.
  """
  def solution(self):
    """
    Return this json object
    """
    return [(("Geographic Names Information System", "Bureau of the Census"), 74.432771208776501)] * 100
